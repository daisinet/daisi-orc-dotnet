...
During heartbeat and environment checks, the Orc queries the active release for the host's release group. If the host's version is older than the active release, an `UpdateRequiredRequest` is sent with the host's Velopack **channel** (the release group name, e.g. `beta`). The host then uses Velopack to self-update from the corresponding channel in Azure Blob Storage (`releases/velopack/{channel}/{rid}/`). If no database release exists for the group, the system falls back to the `Daisi:MinimumHostVersion` config key for backward compatibility.
